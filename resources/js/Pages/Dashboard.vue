                        ...
                        <div class="mb-4">
                            ...
                            <div class="mb-5" v-for="(episode,index) in form.episodes" v-bind:key="index">
                                <label :for="'title-'+index" class="block text-gray-700 text-sm gont-bold nb-2">
                                    Title of the episode n°{{index+1}}
                                </label>
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker"
                                ...
                                <label :for="'description-'+index" class="block text-gray-700 text-sm gont-bold nb-2">
                                    Description of the episode n°{{index+1}}
                                </label>
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker"
                                ...
                                <label :for="'video_url-'+index" class="block text-gray-700 text-sm gont-bold nb-2">
                                    Video's URL of the episode n°{{index+1}} <br>
                                    <em class="text-red-600">(Must be a "embed" youtube link in order to access video's duration. ex: https://www.youtube.com/embed/D3oivlcoEvw)</em>
                                </label>
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker"
                                    :id="'video_url-'+index" type="text" v-model="form.episodes[index].video_url">
                            </div>
                        </div>
                        <button class='my-2 py-2 px-4 bg-green-600 text-white max-w-min rounded block'
                            @click.prevent="add" v-if="form.episodes.length<15">+</button>
                        <button class='my-2 py-2 px-4 bg-red-600 text-white max-w-min rounded block'
                            @click.prevent="remove" v-if="form.episodes.length>1">🗑️</button>
